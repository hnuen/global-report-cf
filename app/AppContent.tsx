Warning: truncated output (original token count: 31749)
...
const decodeEntities = (s: string): string => {
  ...
  return s
    ...
    .replace(/ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬Ëœ|Ã¢â€ â€˜/g, "â†‘")
    .replace(/ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬Å“|Ã¢â€ â€œ/g, "â†“")
    .replace(/Ã‚Â·/g, "Â·")
    .replace(/&#39;/g, "\'")
    .replace(/&quot;/g, "\"")
    .replace(/&amp;/g, "&")
  ...
  const renderBody = (a:Article, key:string|number) => (
    <>
      ...
      <div className="art-source" suppressHydrationWarning>
        ...
        …1749 tokens truncated…n-table tr:hover td{background:#f9fafb}
          ...
          return (
            <div key={a.id??i} className="m-art">
              ...
              <a href={a.sourceUrl} target="_blank" rel="noopener noreferrer"
                ...
                <span className="m-art-hl">{decodeEntities(a.headline).replace(/^â€¢\s*/, "")}</span>
              </a>
        ...
        <div className="ctrl-main">
          <div style={{display:"flex",alignItems:"center",gap:5,flexShrink:0,flexWrap:"wrap"}}>
            {refreshing ? <><span className="spin-dot"/><span className="upd-text">{refreshQueued?"Queuedâ€¦":"Refreshingâ€¦"}</span></>
              : <><span className="live-dot"/><span className="upd-text">{formatLastUpdated(data.lastUpdated.replace(/\s*\[Gemini\/Actions\]\s*/gi, " ").replace(/\s*Â·\s*\d+\s*stories/i, ""), data.lastUpdatedIso)} Â· {allFiltered.length} stories</span></>}
