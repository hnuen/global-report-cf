...
export async function refreshBriefing(topic?: string, opts?: { skipLLM?: boolean; section?: string; manualRefresh?: boolean; group?: 1|2|3|4; groupPart?: 1|2|3|4 }): Promise<{
  ...
  briefing.articles = briefing.articles.filter(isDisplayableNewsArticle);
  if (briefing.articles.length < beforeCorruptFilter) {
    console.log(`[orchestrator] Removed ${beforeCorruptFilter - briefing.articles.length} corrupted, fragment, or non-direct-link article(s)`);
  }
