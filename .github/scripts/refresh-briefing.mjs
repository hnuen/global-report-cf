...
function parseTreasuryNews(html) {
  ...
  const out = [];
  const seen = new Set();
    const re = /<a\b[^>]*\bhref\s*=\s*(["'])(\/news\/press-releases\/sb\d+|https:\/\/home\.treasury\.gov\/news\/press-releases\/sb\d+)\1[^>]*>([\s\S]*?)<\/a>/gi;
  let m;
  while ((m = re.exec(html)) !== null) {
      const url = new URL(m[2], "https://home.treasury.gov").href;
      const title = m[3].replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/&#\d+;/g, " ").replace(/&nbsp;/g, " ").replace(/\s+/g, " ").trim();
...
async function commitOfacCache(recentActions, civilPenalties, programs, ofsiNotices = [], europaNews = [], unNotices = [], bbcNews = [], ajNews = [], occNews = [], economicsNews = [], bisNews = [], regionsNews = []) {
  if (!GITHUB_TOKEN || !GITHUB_REPO) {
    console.warn("[ofac-cache] Missing GITHUB_TOKEN or GITHUB_REPOSITORY â€” skipping cache commit");
    return;
  }
  ...
  } catch { /* new file â€” no SHA needed */ }
  ...
  if (res.ok) {
    ...
    console.log(`[ofac-cache] âœ… Committed: ${recentActions.length} recent-actions, ${civilPenalties.length} penalties, ${progCount} programs, ${ofsiNotices.length} OFSI notices, ${europaNews.length} EU items, ${unNotices.length} UN items, ${bbcNews.length} BBC items, ${ajNews.length} Al Jazeera items, ${occNews.length} OCC items, ${economicsNews.length} Fed items, ${bisNews.length} BIS items, ${regionsNews.length} Regions items to ${path}`);
  } else {
...
try {
  ...
  // ground truth â€” Gemini was handed those exact URLs to write from. Any
...
async function saveBriefingWithRetry(payload, merge = false) {
  ...
  if (!saveRes.ok || !saveData.ok) {
    console.warn(`[refresh-briefing] Save attempt 1 failed (${saveRes.status}): ${JSON.stringify(saveData)} â€” retrying in 5s`);
